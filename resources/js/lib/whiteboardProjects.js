...
/**
 * Hauteur `scrollHeight` d’une ligne du champ d’édition (prochaines étapes / commentaires).
 * @see `StickyNoteNode.vue` `.sticky-note__edit-input`
 */
export const STICKY_EDIT_LINE_BASELINE_SCROLL_PX = 28
 ...
 * et optionnellement le surplus des champs liste en édition (texte sur plusieurs lignes).
 * @param {number} [titleExtraScrollPx=0]  max(0, textarea.scrollHeight - STICKY_TITLE_BASELINE_SCROLL_PX)
 * @param {number} [editListWrapExtraPx=0]  somme des max(0, scrollHeight - STICKY_EDIT_LINE_BASELINE_SCROLL_PX) pour les zones d’édition
 */
export function stickyDimensionsFromLists(
  faitsMarquants,
  commentaires,
  titleExtraScrollPx = 0,
  editListWrapExtraPx = 0,
) {
  const count = (arr) =>
    Array.isArray(arr)
  ...
  const extra =
    Math.max(0, Math.round(Number(titleExtraScrollPx) || 0)) +
    Math.max(0, Math.round(Number(editListWrapExtraPx) || 0))
  const base =
    n === 0
